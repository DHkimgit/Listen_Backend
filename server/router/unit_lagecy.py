...
@router.get("/{id}")
async def get_unit(id: int, db: AsyncSession = Depends(get_db_session)):
    query = select(Unit).filter(Unit.id == id)
    execute = await db.execute(query)
    result = execute.scalars().one()
    return result
 
@router.delete("/{id}")
async def delete_unit(id: int, db: AsyncSession = Depends(get_db_session)):
    query = select(Unit).filter(Unit.id == id)
    execute = await db.execute(query)
    delete = await db.delete(execute)
    db.commit()
    return "rer"
    if execute:
